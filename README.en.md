...
As of v72, both the source repository and the installed copy pass 126 regression tests (the prior baseline was 120, with bilingual semantic, editable-node, invalid-input and viewport-contract coverage added for Lean Canvas, user-path storyboard and the demo shell). All 23 demo pages were measured in real 390×844, 768×1024, 1280×720 and 1600×900 browser viewports; page-level horizontal overflow was 0/92. Fifteen representative draw.io files have now been edited, saved and reopened in diagrams.net, covering architecture, swimlane, sequence, bar chart, bilingual Lean Canvas, user-path storyboard, education experience maps and tree/mind-map structures. This does not certify native editing for every form or a JEV review; see the [verification record](references/verification-status.md).
