...
For a first trial, use the [bilingual six-industry trial pack](references/trial-guide.en.md), make one scenario revision, and inspect the SVG, HTML, draw.io, and JSON boundaries; the [v74 release notes](references/release-notes-v74.en.md) define the tested scope and limits.
...
As of v74, both the source repository and the installed copy pass 130 regression tests (the prior baseline was 128; the added checks verify trial receipt hashes, scene/QA parsing, visible mutation text, swimlane roles and retail/marketing semantics). All 23 demo pages were measured in real 390×844, 768×1024, 1280×720 and 1600×900 browser viewports; page-level horizontal overflow was 0/92. Embedded content on nine popular pages—flow, architecture, swimlane, Gantt, network, data story, story map, user-path storyboard and Lean Canvas—was additionally measured at 390×844 and 768×1024, with page overflow 0/18. Narrow viewports default to readable natural-size drawings inside explicit scroll regions; measured embedded text is 14px minimum, with a declared 12px floor for the compact Gantt print variant. Fifteen representative draw.io files have now been edited, saved and reopened in diagrams.net, covering architecture, swimlane, sequence, bar chart, bilingual Lean Canvas, user-path storyboard, education experience maps and tree/mind-map structures. Six cross-industry trial cases now each have a revised JSON, SVG, draw.io, scene, QA and delivery receipt, with the regression contract checking the input hash, receipt manifest, QA status and rendered mutation text. This does not certify native editing for every form or a JEV review; see the [verification record](references/verification-status.md).
