...
As of v72, both the source repository and the installed copy pass 124 regression tests (the prior baseline was 120, with bilingual semantic, editable-node, and invalid-input coverage added for Lean Canvas and user-path storyboard). Eleven overviews have passed actual desktop-browser text checks, and workflow/story-map samples have been edited, saved and reopened in draw.io; the two new types have generated structural evidence, with native-editor round trips stated separately. This is not approval of all diagram types, mobile layouts or aesthetics; see the [verification record](references/verification-status.md).
