...
def render_svg(model, geometry):
    ...
    parts = [
        ...
        svg_text(1550, 47, "快照 " + model["snapshot"], 12, MUTED, 450, "end"),
    ]
    metrics = model["flow_metrics"]
    pills = [
        ("在制", str(model["counts"]["wip"])),
        ("阻塞", str(model["counts"]["blocked"])),
        ("吞吐", f'{metrics["throughput"]} {metrics["throughput_unit"]}'),
        ("SLE", f'{int(metrics["sle_probability"]*100)}% ≤ {metrics["sle_days"]}天'),
    ]
    px = 815
    ...
    for lane_index, lane in enumerate(model["lanes"]):
        ...
        parts.append(svg_text(110, y + 68, f'泳道WIP {lane_count}/{lane["wip_limit"]}', 10, MUTED, 500, "middle"))
        ...
        for col_index, column in enumerate(model["columns"]):
            ...
            for card_index, card in enumerate(g["by_cell"][(lane["id"], column["id"]) ]):
                ...
                if card["blocked"]:
                    ...
                    parts.append(svg_text(cx + cw - 34, cy + 23, "阻塞", 10, "#FFFFFF", 650, "middle"))
                    parts.append(svg_text(cx + cw - 12, cy + 52, card["blocked_reason"], 9, BLOCKED, 500, "end"))
                else:
                    parts.append(svg_text(cx + cw - 12, cy + 22, CLASS_LABELS[card["service_class"]], 9, CLASS_COLORS[card["service_class"]], 600, "end"))

    footer_y = g["footer_y"]
    parts.append(svg_text(50, footer_y, "显式策略", 18, INK, 620))
    parts.append(svg_text(1550, footer_y, "阻塞项仍计入WIP · 下游有容量才拉动", 11, MUTED, 500, "end"))
    for index, policy in enumerate(model["policies"]):
        ...
        for line_index, line in enumerate(wrap(policy["rule"], 42)[:2]):
            parts.append(svg_text(x + 90, y + 23 + line_index * 17, line, 11, INK, 400))
    parts.append(svg_text(50, g["height"] - 17, "教学模拟数据 · 泳道表示工作类型，不用于个人绩效排名", 10, MUTED, 450))
    parts.append("</svg>")
    return "\n".join(parts) + "\n"
...
def render_drawio(model, geometry):
    ...
    diagram = ET.SubElement(mxfile, "diagram", id="kanban", name="泳道看板")
    ...
    metrics = model["flow_metrics"]
    metric_values = [
        f'在制\n{model["counts"]["wip"]}', f'阻塞\n{model["counts"]["blocked"]}',
        f'吞吐\n{metrics["throughput"]} {metrics["throughput_unit"]}',
        f'SLE\n{int(metrics["sle_probability"]*100)}% ≤ {metrics["sle_days"]}天',
    ]
    ...
    for index, column in enumerate(model["columns"]):
        ...
        if column["stage"] == "wip":
            value += f'<br><font color="{MUTED}">WIP {model["column_wip"][column["id"]]}/{column["wip_limit"]}<br>{html.escape(column["pull_criteria"])}</font>'
        else:
            value += '<br><font color="' + MUTED + '">' + ("承诺前" if column["stage"] == "queue" else "交付完成") + '</font>'
    ...
    for lane_index, lane in enumerate(model["lanes"]):
        ...
        value = f'<b>{html.escape(lane["name"])}</b><br><font color="{MUTED}">WIP {model["lane_wip"][lane["id"]]}/{lane["wip_limit"]}</font>'
        ...
        for col_index, column in enumerate(model["columns"]):
            ...
            for card_index, card in enumerate(g["by_cell"][(lane["id"], column["id"]) ]):
                ...
                if "due" in card:
                    meta += f' · 截止{card["due"]}'
                extra = f'<br><font color="{BLOCKED}">阻塞：{html.escape(card["blocked_reason"])}</font>' if card["blocked"] else f'<br><font color="{CLASS_COLORS[card["service_class"]]}">{CLASS_LABELS[card["service_class"]]}</font>'
    ...
    footer_y = g["footer_y"]
    vertex("policy-title", "显式策略", 50, footer_y - 10, 300, 30, plain + f"fontSize=18;fontStyle=1;fontColor={INK};")
    for index, policy in enumerate(model["policies"]):
        row, col = divmod(index, 2)
        x, y = 50 + col * 755, footer_y + 25 + row * 76
