...
- 六个热门演示外壳在窄视窗默认进入可读原尺寸模式；实际嵌入正文最小显示字号为 14px，甘特紧凑打印版单独记录为 12px，页面级横向溢出仍为 0/18。
- 软件、电商、教育、营销、人事行政、文旅/门店六个跨行业试用案例各完成一次有意义的修改，并保留输入、SVG 预览、draw.io 源、scene、QA 与 delivery receipt。
- 试用包回归现在验证输入 SHA-256、receipt 清单哈希、scene/QA 可解析且无错误，并从 SVG 文本确认实际修改文字已渲染；泳道角色和零售/营销场景也有语义回归。
...
源仓库与安装目录各通过 130 项回归；quick validation、JSON 解析、链接审计、差异检查和目录 parity 均需在发布前再次运行并记录。基础 renderer 只依赖 Python 3.9+；跨行业等专门后端可能需要 Matplotlib/Pillow。

## 边界

模拟案例不等于行业事实，专业领域仍需人工复核；热门窄屏证据不外推到任意高密度输入。选图连续性证据覆盖声明的状态保留、分类记忆、键盘顺序和格式边界，不等同于 390px 全键盘路径或字节级剪贴板验收。万兴图示对本机临时 `.drawio` 副本仍显示“打开文件失败”，因此只记录 diagrams.net 代表性往返通过。JEV 没有真实 Skill 名称、路径或可调用入口，暂不写成已接入或已通过。
