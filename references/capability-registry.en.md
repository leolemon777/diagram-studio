...
The machine-readable source is [`assets/capability-registry.json`](../assets/capability-registry.json) (v72). The chooser reads structure, backend, formats and evidence from this file. Run `python3 scripts/capability_registry.py --check` to validate every backend, example and evidence reference. The compact table below is a human index; the JSON carries the complete fields.
...
| `lean-canvas` | Lean canvas | `scripts/layouts.py#lean-canvas` | SVG, draw.io, scene/QA, HTML | `generated_with_semantic_validation` |
...
| `user-path-storyboard` | User-path storyboard | `scripts/user_path_storyboard.py#user-path-storyboard` | SVG, draw.io, scene/brief/QA, HTML | `generated_with_semantic_validation` |
